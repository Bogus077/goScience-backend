...
export const updatePointsMonth = async (
  ...
  const taskQuarter = await TasksQuarter.findOne({
    where: { id: TasksQuarterId },
  });
  if (taskQuarter) {
    await taskQuarter.update({
      points:
        type === 'add'
          ? taskQuarter.points + points
          : taskQuarter.points - points,
    });
  }
};

export const updatePointsWeek = async (
  TasksMonthId: number,
  points: number,
  type: 'add' | 'remove'
) => {
  ...
  if (taskMonth) {
    await taskMonth.update({
      points:
        type === 'add' ? taskMonth.points + points : taskMonth.points - points,
    });

    if (taskMonth.TasksQuarterId)
      await updatePointsMonth(taskMonth.TasksQuarterId, points, type);
  }
};

export const updatePointsDay = async (
  TasksWeekId: number,
  points: number,
  type: 'add' | 'remove'
) => {
  ...
  if (taskWeek) {
    await taskWeek.update({
      points:
        type === 'add' ? taskWeek.points + points : taskWeek.points - points,
    });

    if (taskWeek.TasksMonthId)
      await updatePointsWeek(taskWeek.TasksMonthId, points, type);
  }
};
...
export const getKidTasks = async (requestData: Request['body']) => {
  ...
  const KidTasks = await Kid.findOne({
    where: { id: requestData.KidId },
...
const getKidTaskWeekId = async (
  ...
  if (!taskWeek) {
    errors.push(
      `Недельная задача с идентификатором ${task.TasksWeekId} не найдена`
    );
    return null;
  } else {
    const taskWeekId = await TasksWeek.findOne({
      where: { taskgroupId: taskWeek.taskgroupId, KidId: task.KidId },
    });
    if (!taskWeekId) {
      const newTaskWeek = {
        KidId: task.KidId,
      ...
      const newWeek = await createWeekTask(
        newTaskWeek,
        taskWeek.taskgroupId,
        errors,
        kid
      );

      errors.push(
        `Недельная задача ${taskWeek.label} не найдена у ученика ${kid.surname} ${kid.name}. Поэтому она была создана в процессе добавления данной задачи.`
      );
      return newWeek.id;
    }
...
const createDayTask = async (
  ...
  const newTask = await TasksDay.create({
    ...task,
...
const getKidTaskMonthId = async (
  task: Request['body'],
  errors: string[],
  kid: typeof Kid
) => {
  const taskMonth = await TasksMonth.findOne({
    where: { id: task.TasksMonthId },
  });

  if (!taskMonth) {
    errors.push(
      `Месячная задача с идентификатором ${task.TasksMonthId} не найдена`
    );
    return null;
  } else {
    const taskMonthId = await TasksMonth.findOne({
      where: { taskgroupId: taskMonth.taskgroupId, KidId: task.KidId },
    });
    if (!taskMonthId) {
      const newTaskMonth = {
        KidId: task.KidId,
      ...
      const newMonth = await createMonthTask(
        newTaskMonth,
        taskMonth.taskgroupId,
        errors,
        kid
      );

      errors.push(
        `Месячная задача ${taskMonth.label} не найдена у ученика ${kid.surname} ${kid.name}. Поэтому она была создана в процессе добавления данной задачи.`
      );
      return newMonth.id;
    }
...
const getKidTaskQuarterId = async (
  task: Request['body'],
  errors: string[],
  kid: typeof Kid
) => {
  const taskQuarter = await TasksQuarter.findOne({
    where: { id: task.TasksQuarterId },
  });

  if (!taskQuarter) {
    errors.push(
      `Четвертная задача с идентификатором ${task.TasksQuarterId} не найдена`
    );
    return null;
  } else {
    const taskQuarterId = await TasksQuarter.findOne({
      where: { taskgroupId: taskQuarter.taskgroupId, KidId: task.KidId },
    });
    if (!taskQuarterId) {
      ...
      const newQuarter = await createQuarterTask(
        newTaskQuarter,
        taskQuarter.taskgroupId,
        errors
      );

      errors.push(
        `Четвертная задача ${taskQuarter.label} не найдена у ученика ${kid.surname} ${kid.name}. Поэтому она была создана в процессе добавления данной задачи.`
      );
...
export const createTask = async (
  ...
  if (!requestData.KidId || requestData.KidId.length === 0)
    throw { errorMessage: 'Не выбран ученик, которому добавляется задание' };
...
export const addDayToTask = async (
  requestData: Request['body'],
  UserId: number
) => {
  validateData(requestData, addDayToTaskRules);
  ...
  today.setMinutes(0);
  today.setSeconds(0);
  const nextDay = new Date(taskDay.date.getTime() + 1000 * 60 * 60 * 24);
  const isSameDay = taskDay.date.getDate() === today.getDate();
  const newTaskDayDate = isSameDay ? new Date(nextDay) : today;

  // console.log('=============================')
  // console.log('=============================')
  // console.log('=============================')
  // console.log(taskDay.date)
  // console.log(today)
  // console.log('=============================')
  // console.log('=============================')
  // console.log('=============================')

  return await taskDay.update({ date: newTaskDayDate });
};
  ...
  switch (requestData.type) {
    case 'day':
      ...
      if (
        ...
        const statsToDelete = await StatsTask.findOne({
          where: { TasksDayId: taskDay.id },
        });
        if (statsToDelete) await statsToDelete.destroy();
      }

      //stats
      await StatsTask.create({
        UserId,
        KidId: taskDay.KidId,
        TasksDayId: taskDay.id,
        points: taskDay.points,
      });
      await KidSummaryTask.create({
        KidId: taskDay.KidId,
        label: taskDay.label,
        points: taskDay.points,
      });
    ...
    case 'week':
      const taskWeek = await TasksWeek.findOne({
      ...
      if (requestData.status === true && taskWeek.status === false) {
        await KidSummaryTask.create({
          KidId: taskWeek.KidId,
          label: taskWeek.label,
          points: taskWeek.points,
        });
      }
    ...
    case 'month':
      ...
      if (requestData.status === true && taskMonth.status === false) {
        await KidSummaryTask.create({
          KidId: taskMonth.KidId,
          label: taskMonth.label,
          points: taskMonth.points,
        });
      }
    ...
    case 'quarter':
      ...
      if (requestData.status === true && taskQuarter.status === false) {
        await KidSummaryTask.create({
          KidId: taskQuarter.KidId,
          label: taskQuarter.label,
          points: taskQuarter.points,
        });
      }
